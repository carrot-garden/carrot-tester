package com.carrotgarden.hdf5.example;
...
public class H5Ex_T_Compound {
	...
	private static String FILENAME = "target/h5ex_t_cmpd.h5";
	...
	private static class CompoundDatatype {
		...
		protected static final int[] MAGICNUMBERVALUE = { 0xac, 0xed, 0x00,
				0x05 };
	...
	private static class Sensor_Datatype extends CompoundDatatype {
		...
		static int[] memberStorage = { OBJHEADERSIZE, INTEGERSIZE,
	...
	private static void CreateDataset() {
			...
			if (memtype_id >= 0) {
				for (int indx = 0; indx < Sensor_Datatype.numberMembers; indx++) {
					...
					H5.H5Tinsert(memtype_id,
							Sensor.datatypes.memberNames[indx],
		...
		try {
			ByteArrayOutputStream baos = new ByteArrayOutputStream(
					Sensor_Datatype.getTotalDataSize());
			...
			byte[] write_data = new byte[dset_data.length
					- CompoundDatatype.MAGICNUMBER];
			for (int indx = 0; indx < dset_data.length
					- CompoundDatatype.MAGICNUMBER; indx++)
				write_data[indx] = dset_data[indx
						+ CompoundDatatype.MAGICNUMBER];
	...
	private static void ReadDataset() {
		...
		byte[] read_data = new byte[(int) dims[0]
				* Sensor_Datatype.getDataSize()];
		...
		try {
			...
			dset_data = new byte[read_data.length
					+ CompoundDatatype.MAGICNUMBER];
		...
		for (int indx = 0; indx < dims[0]; indx++) {
			...
			System.out.println("Serial number   : "
					+ object_data2[indx].serial_no);
			System.out.println("Location        : "
					+ object_data2[indx].location);
			System.out.println("Temperature (F) : "
					+ object_data2[indx].temperature);
			System.out.println("Pressure (inHg) : "
					+ object_data2[indx].pressure);
			System.out.println();
		}
		System.out.println();
