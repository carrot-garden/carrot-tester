...
public class AetherTest {
	...
	@Test
	public void resolveRangeBased() throws DependencyCollectionException,
			ArtifactResolutionException, IOException {
		final AetherBasedResolver aetherBasedResolver = new AetherBasedResolver(
				getDummyConfig());
		aetherBasedResolver.resolve("org.ops4j.pax.web", "pax-web-api", "",
	...
	@Test
	public void testCachingOfRanges() throws DependencyCollectionException,
			ArtifactResolutionException, IOException {

		final MavenConfiguration config = getDummyConfig();

		AetherBasedResolver aetherBasedResolver = new AetherBasedResolver(
				config);
		aetherBasedResolver.resolve("org.ops4j.pax.web", "pax-web-api", "",
				"jar", "LATEST").close();
	...
	private MavenConfiguration getDummyConfig() throws IOException {

		final Properties p = new Properties();

		p.setProperty(ServiceConstants.PID
				+ MavenConstants.PROPERTY_LOCAL_REPOSITORY, getCache().toURI()
				.toASCIIString());

		p.setProperty(
				ServiceConstants.PID + MavenConstants.PROPERTY_REPOSITORIES,
	...
	private File getCache() throws IOException {
		...
		final File f = File.createTempFile("aethertest", ".dir", base);
	...
	@Test
	public void testResolveRDF() throws DependencyCollectionException,
			ArtifactResolutionException, IOException {
		final Properties p = new Properties();
		p.setProperty(ServiceConstants.PID
				+ MavenConstants.PROPERTY_LOCAL_REPOSITORY, getCache().toURI()
		...
		final MavenConfigurationImpl conf = new MavenConfigurationImpl(
				new PropertiesPropertyResolver(p), ServiceConstants.PID);

		final AetherBasedResolver aetherBasedResolver = new AetherBasedResolver(
				conf);
		aetherBasedResolver.resolve("org.openrdf", "openrdf-model", "", "jar",
				"2.0.1").close();
